...
res = run_application_pipeline(db, job_id=job_id, template_name="ai.tex", mark_applied=False)
print("Pipeline result for Scale AI:", res)
...
cl_md_path = GEN_DIR / "Scale_AI_cover_letter.md"
if cl_md_path.exists():
    ...
    cl_tex_path = GEN_DIR / "Scale_AI_cover_letter.tex"
    cl_pdf_path = GEN_DIR / "Scale_AI_cover_letter.pdf"
    with open(cl_tex_path, "w", encoding="utf-8") as f:
        f.write(latex_str)
    compile_latex_to_pdf(str(cl_tex_path))
    print("Scale AI Cover Letter PDF compiled:", cl_pdf_path.exists())
